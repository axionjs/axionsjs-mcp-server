  ...
  try {
    ...
    switch (request.params.name) {
      case "init_axions_project": {
        const { style = "new-york", includeTheme = true } =
...
\`\`\``;
      ...
      case "get_component_list": {
        const { category, type, registryType } = request.params.arguments;
        let registryIndex = await getAxionsRegistryIndex();
        if (!registryIndex) {
          return {
...
${metadata.item.tags?.join(", ") || "None"}`;
      ...
      case "install_components": {
        const { components, overwrite, path, style } = request.params.arguments;
      ...
      case "resolve_dependencies": {
        const { components, includeDevDeps = false } = request.params.arguments;
      ...
      case "generate_component_code": {
        const { component, style, customProps, includeExamples } =
          request.params.arguments;
...
Example:
\`\`\`bash
npx axionjs-ui add blocks hero-section
npx axionjs-ui add auth login-form
npx axionjs-ui add charts line-chart
\`\`\`
`,
            },
          ],
