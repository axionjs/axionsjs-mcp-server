...
import { getAxionsRegistryIndex, getAxionsRegistryStyles, searchAxionsComponents, getAxionsComponentMetadata, resolveAxionsComponentTree, generateAxionsInstallCommand, getAllAxionsComponents, getAxionsRegistryThemes, getAxionsRegistryDynamicComponents, getAxionsThemeByName, getAxionsDynamicComponentByName, 
// New MagicUI-style functions
fetchUIComponents, fetchComponentDetails, fetchExampleComponents, fetchExampleDetails, fetchComponentsByType, findComponentByName, } from "../lib/registry-api.js";
    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "Fetch components by their registry type from the unified registry",
                inputSchema: zodToJsonSchema(z.object({
                    type: z
                        .string()
            ...
            },
            {
                ...
                description: "Find a specific component by name across all registry types",
                inputSchema: zodToJsonSchema(z.object({
...
${metadata.item.tags?.join(", ") || "None"}`;
            ...
            case "install_components": {
                ...
                const { resolved, dependencies } = await resolveAxionsComponentTree(components, style);
            ...
            case "resolve_dependencies": {
                const { components, includeDevDeps = false } = request.params
                    .arguments;
                const { resolved, dependencies, installCommand } = await resolveAxionsComponentTree(components, "new-york");
                        ...
                        content: [
                            {
                                ...
                                text: `Found ${examples.length} example components:\n\n${examples
                                    .map((example) => `**${example.name}** (${example.type})\n${example.description || "No description"}${example.registryDependencies?.length
                        ...
                        content: [
                            {
                                ...
                                text: `Found ${components.length} components of type "${type}":\n\n${components
                                    .map((comp) => `**${comp.name}**\n${comp.description || "No description"}${comp.dependencies?.length
