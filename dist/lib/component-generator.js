...
export async function generateComponentCode(componentName, options = {}) {
    ...
    const { dependencies, installCommand } = await resolveAxionsComponentTree([componentName], options.style || "new-york");
    return {
        code,
        dependencies,
        installCommand,
    ...
    try {
        ...
        for (const componentName of components) {
            ...
            if (component && registryType) {
                ...
                const typedRegistryType = registryType;
                resolvedComponents.push({ component, registryType: typedRegistryType });
                const { installCommand } = await resolveAxionsComponentTree([componentName], options.style || "new-york");
        ...
        if (resolvedComponents.length === 0) {
            ...
            if (pageType === "hero" && allRegistryComponents.block) {
                const heroBlocks = allRegistryComponents.block.filter((block) => block.name.toLowerCase().includes("hero"));
                if (heroBlocks.length > 0) {
                    for (const block of heroBlocks.slice(0, 3)) {
                        ...
                        const { installCommand } = await resolveAxionsComponentTree([block.name], options.style || "new-york");
                        installCommands.push(installCommand);
                    }
                }
            }
            ...
            for (const { component, registryType } of relevantComponents) {
                ...
                const { installCommand } = await resolveAxionsComponentTree([component.name], options.style || "new-york");
                installCommands.push(installCommand);
            }
        }
